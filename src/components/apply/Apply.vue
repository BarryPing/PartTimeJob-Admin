      ...
      <el-table id="exportTab" v-loading="loading" :data="applylist" border stripe>
        ...
        <el-table-column label="处理状态">
          <template v-slot:="scope">
            <el-tag v-if="scope.row.aocState===1" type="success">已同意</el-tag>
            <el-tag v-else-if="scope.row.aocState===2" type="danger">已拒绝</el-tag>
        ...
        <el-table-column label="操作 ">
          <template v-slot:="scope">
            ...
            <el-tooltip effect="dark" content="处理流程" placement="top" :enterable="false">
              <el-button type="success" icon="el-icon-location-outline" size="mini" @click="showProgressBox(scope.row.aocHandleid)" />
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
      ...
      >
        <el-form :model="applyDetails" label-width="100px" label-position="top">
          ...
          <el-form-item label="申请状态">
            <el-tag v-if="applyDetails.aocState===1" type="success">已同意</el-tag>
            <el-tag v-else-if="applyDetails.aocState===2" type="danger">已拒绝</el-tag>
            <el-tag v-else type="warning">未处理</el-tag>
          </el-form-item>
